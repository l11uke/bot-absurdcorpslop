...
module.exports = NodeHelper.create({
  start() {
    console.log("MMM-AntiAd helper started");
  },

  async socketNotificationReceived(notification, payload) {
    if (notification === "GET_AD") {
      try {
        const ad = await this.generateAd();
        this.sendSocketNotification("AD_RESULT", ad);
      } catch (e) {
        console.error("MMM-AntiAd error:", e);
        this.sendSocketNotification("AD_ERROR", e.message);
      }
    }
  },
  ...
  async generateAd() {
    ...
    const [a, b] = await Promise.all([this.getWikiArticle(), this.getWikiArticle()]);
